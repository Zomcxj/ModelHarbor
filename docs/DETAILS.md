...
- **数字字段解析失败时一律不写，绝不写成 `0`。** 界面把非法输入标红并提示「保存时该字段将被忽略」，保存状态栏也会报「已忽略 N 个无效数字字段」——写 `0` 是另一回事：`contextWindow: 0` / `maxInputTokens: 0` 会被 agent 照单全收，那个模型的上下文直接归零。落笔统一走「解析成功才 `insert`」（`if let Ok(n) = text.parse::<i64>()`，与 opencode 系 `set_num_opt`、pi 的 `model_to_pi`、DSH 的 `parse_number_text` 同一口径）。ZCode / WorkBuddy 两个后端曾经用 `unwrap_or(0)`，是这一版修掉的。
