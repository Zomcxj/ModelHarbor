...
- **放行开关**：标题行**下方单独一行、右对齐**有「**代理支持**」勾选框（写入 `settings.json` 的 `allow_model_test_with_proxy`，默认 `false`）。它是个安全开关，和标题行那排视图按钮分开摆，既不易误点也看得出因果关系；警告 / 已放行文案与它同排（在它左边）。勾上后即使检测到代理 / VPN 也允许模型延迟测试，警告文案转为「已放行」
...
部分站点（new-api 的「签到」功能）每天可领一份随机额度。它的接口是：

- `GET  /api/user/checkin`：今天是否已签、本月 / 累计次数、累计获得额度（只读）；
- `POST /api/user/checkin`：执行签到（**写操作**）。

**入口在卡片上**：厂商卡片右侧、「复制」左边有「签到」按钮——该站点**填了面板访问令牌就自动出现**，
不需要额外勾选、也没有需要记住的开关。它仍与「查询用量」完全分开：
签到会**改变账号额度**并让站点记一条系统日志，是本工具唯一会改服务端数据的功能，
所以只在你按下这个按钮时发生，一键「查询用量」永远不会顺带签到。

点击后的行为：先读状态，**今天没签才执行签到**。

- 已签到：显示「今日已签到，获得 $X（本月 N 次，累计获得 $Y）」，不再发 POST。
  今日金额取自站点 `stats.records` 里**日期最大**的那条（`YYYY-MM-DD` 字典序即时间序，
  不依赖站点给的先后顺序），并且只在 `checked_in_today` 为真时才当「今天」——
  否则宁可不说，也不把别的日子报成今日；
- 今天没签：执行签到并显示「签到成功，获得 $X」；
- 站点没开该功能：显示站点原话「签到功能未启用」；
- 站点开了 **Cloudflare 人机验证**（new-api 的 `TurnstileCheck` 中间件）：
  `POST` 必须带浏览器解题产生的 `?turnstile=` token，本工具不做人机验证，
  会把站点原话加上「只能在浏览器里签到」报出——这是站点的反自动化措施，不是缺陷。

签到需要**面板访问令牌**（`sk-` key 不够，站点要的是用户身份）；部署旧版 new-api 的站点同样需要「用户 ID」。
请求同样是直连、只把凭证放进请求头。
...
- 保存按钮这一行**右端**是全局「显示密钥 / 隐藏密钥」（一键切换全部 API Key 的明文 / 掩码）。放在这里而不是 Providers 标题行：它管的是**所有页面**的密钥显示，而标题行只有切到 Providers 页才看得到
