...
- 写入策略：内容没有变化就不写盘；读不出 / 解析失败 / 文件不存在都按默认值处理（界面设置坏了不该影响工具可用性）
- 取不到家目录时回退到 `%APPDATA%\.modelharbor\`，再回退到程序同级目录；任何情况下都不会写进 agent 配置目录
- 删除该文件即恢复默认界面设置

### 卡片折叠状态

- 记录的是**折叠**的卡片（`providers/名字`、`agents/名字`），不在记录里的就是展开：新加载的卡片默认展开
- 键不带页面前缀，同一份配置在四个页面里是同一批卡片，折叠状态跟着卡片走
- 加载成功后清理当前配置里已不存在的记录（删掉 / 改名后不残留）；加载失败时保留，不会因为路径临时不可用而丢掉状态
...
写盘只写新位置的新名字，并在首次保存成功后清掉同目录下的旧文件（老设置不会丢）。
