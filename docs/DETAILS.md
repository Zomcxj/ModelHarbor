...
上下文字段右侧的预设下拉给出常用的上下文值（128k / 200k / 256k / 272k / 300k / 500k / 1024k），输出字段右侧同理（32k / 64k / 128k / 256k）。选中即填入，**填完仍可继续手动编辑**——它只是省去手敲大数字，不锁定字段。

预设按 **1000 进制**换算成配置里的整数（`128k` → `128000`），与仓库既有的写法一致（新模型的占位值就是 `272000`）。不用 1024 是因为这些字段是**发给上游的声明**：厂商文档与网关界面普遍按 1000 报数，少声明一点是安全的，多声明会被上游直接拒绝，所以 `1024k` 落 `1024000` 而不是 `1048576`。
