...
前两个接口**互相独立**：有些站点的 baseUrl 指向的是中转域名，只挂了 relay 路由，`/api/usage/token/` 会回 `Invalid URL`，但 `/api/log/token` 照样可用。这种情况下今日 / 近 7 天与模型拆分照常显示，只是没有额度三项，不会因为额度接口缺失就把日志统计一起丢掉（小窗也不为此写一行解释）。两个接口都拿不到时才回退兼容账单。
...
悬停详情只放**数字与影响读数的口径**（换算比假设、跳日提醒、`已用` 是账号累计值、`今日` 从本机 0 点算）：
接口来源（`/api/user/self`、`/api/log/token`）、只读说明、日志分页上限、降级原因（`备注`）、
「额度是占位值所以不显余额」「该站点未提供令牌额度接口」这类解释一律**不写**。缺数据不产生数字，
再写一句解释只是噪音；查不到（401 / WAF / 超时 / 未开放接口）一律**静默**，只在状态栏报个数。
小窗只放数据。（“签到要不要签”这类主动作另有可见的状态词，不靠悬停。）
...
- 站点没开签到、没填面板令牌、接口读不到（404 / WAF / 空数据）→ **整段不显示**，不写占位、也不报错；
...
提示 / 淡色小字（输入框占位提示、`.weak()` 小字）的颜色由主题按调色板算出
（`theme::Palette::hint_color`）：标准是「**和禁用的『删除』一个颜色**」。
禁用不是换一个灰，而是 egui 把整棵子树按 `disabled_alpha`（0.5）淡化，所以这里把同一个
算式算到不透明为止：`正文 / 控件底色 / 面板底色` 三个值合出禁用文字的观感（暗色 `#848484`）：
输入框占位提示画在输入框底色上，只有预先合成为同一个 RGB，看上去才真的是一个颜色。
（早先的做法是设 `override_text_color` 统一正文色，而纯文本 `WidgetText` 取色是
`override_text_color.unwrap_or(PLACEHOLDER)`——它把 `hint_text` 的淡色（作为兜底色传入）
一并挡住了，提示实际画的是正文色，看上去像已经填好的值。现在正文色写在各状态的
`fg_stroke` 上，不再拦兜底色。）
