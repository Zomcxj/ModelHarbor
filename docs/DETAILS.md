...
- **放行开关**：标题行有「代理下测试模型」勾选框（写入 `settings.json` 的 `allow_model_test_with_proxy`，默认 `false`）。勾上后即使检测到代理 / VPN 也允许模型延迟测试，警告文案转为「已放行」
...
**入口在卡片上**：厂商卡片右侧、「复制」左边有「签到」勾选框；勾选后才出现「签到」按钮。

为什么默认不勾、且与「查询用量」完全分开：签到会**改变账号额度**并让站点记一条系统日志，
是本工具唯一会改服务端数据的功能，所以它只在用户明确勾选并点击时发生——
一键「查询用量」永远不会顺带签到。勾选状态按配置文件分区存在 `settings.json` 的 `checkin_enabled`
（与折叠状态分开存：卡片被删时不会连带清掉授权）。

点击后的行为：先读状态，**今天没签才执行签到**。

- 已签到：显示「今日已签到（本月 N 次，累计获得 $X）」，不再发 POST；
