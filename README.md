...
- **用量 / 余额查询**：对有 `sk-` key 的 provider 发起只读管理请求，优先令牌额度与调用日志接口，失败再回退兼容账单接口；直连不走系统代理，同一 provider 查询间隔 ≥5 秒；占位额度不显示虚假余额，查不到或空数据不显示结果，今日数据按本机时区计算并在跨日后失效
...
- **界面设置持久化**：主题、保存格式、密钥显隐、WSL 同步、卡片折叠、各页路径写在 `%USERPROFILE%\.modelharbor\settings.json`——只存界面选择，不存密钥与配置内容
